...
under `.workflow/<folder>` holding a plugin's persisted artifacts, gated by a
marker file so only explicitly installed locations are ever read. `.workflow`
is this marketplace's own root and sits exactly where the agent CLI's
`.claude` config dir sits — as its sibling — so plugin data is never mixed
into the CLI's config. The factory
...
| `global` | `~/.workflow/<folder>/<projectName>` | per-project namespace under home |
| `shared` | `~/.workflow/<folder>-shared` | cross-project; always `projectName: null` |

## Decision matrix

| Config key | Type | Effect |
|---|---|---|
| `folder` | string (required) | Bare plugin name; every tier materializes it on disk under `ROOT_DIR` as `.workflow/<folder>`. The shared tier derives `<folder>-shared`, a **sibling** of the per-project namespace, so a project named like the shared folder can never shadow it. |
...
| `ancestorWalkStopsAtHome` | boolean (default `false`) | `true`: the worktree walk checks each directory's marker FIRST, then stops at the user's home — a marker AT `~/.workflow/<folder>` stays discoverable, but the walk never continues PAST home, so a sandboxed `$HOME` cannot leak the real user's store. `false`: walk to the filesystem root. Exhaustion returns `''` either way. |
...
```
{ MARKER, FOLDER, SHARED_FOLDER, ROOT_DIR, PRECEDENCE_ORDER,
...
tier stamped `projectName: null`. `ROOT_DIR` (`.workflow`) is the marketplace
root every tier lives under, for callers that assemble a store path
themselves. `candidateStores(cwd, projectName)` returns
...
Multiple plugins persist per-project state under `.workflow/<folder>` and had
