...
| `storeMigration` | Versioned, fail-open, lock-guarded migration of those store directories: a `.version.json` stamp per store, a `migrations/` folder of `<timestamp>_<slug>.js` files discovered at load, and the move-a-store body (`relocateDirectory`). Its `lock.js` is the repo's shared exclusive-create file lock — also used by the maestro conductor and work-workflow's task claims. |
