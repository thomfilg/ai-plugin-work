...
4. **`request_user_input` availability** — one agent labeled it experimental/off; the live exec model-reported toolset includes `functions.request_user_input`. Favor the live list: PRESENT in 0.142.5 exec default toolset; its schema/UX vs Claude's AskUserQuestion differs regardless. **AMENDED (2026-07-07): listed ≠ callable — the tool is Plan-mode-only.** Calling it in code/default mode errors with "request_user_input is unavailable in code mode" (openai/codex#10384; #29104 requests default-mode availability, still open). Vocabulary/docs therefore default codex question gates to plain-chat numbered options; `request_user_input` is mentioned only for Plan-mode flows.
