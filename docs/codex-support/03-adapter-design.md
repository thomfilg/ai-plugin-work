...
| C3 | No `AskUserQuestion`; codex analog `request_user_input` has a different schema; nothing interactive in unattended exec (§9.4) | runtime + mode | TUI: render `askUserQuestionPayload` as prose + "call `request_user_input`". Exec: park step BLOCKED, persist hold file; answer channel = maestro `/signal` inbox or `codex exec resume <SESSION_ID> "<answer>"` (positional PROMPT arg live-verified in WP-12 — GT §11.3; `--last` variant also valid; `-s`/`-C` rejected, set sandbox via `-c sandbox_mode=...`) | `[work:codex-degraded] interactive gate parked — answer via maestro signal or codex exec resume` |
...
//   channel matrix: UPS/SessionStart → stdout (both runtimes; codex DROPS bracket/JSON-looking
//   stdout — guardStdoutContext() rewrites the lead-in; envelope channel racy under sibling-hook
//   failure, do not rely on it — GT §11.5);
