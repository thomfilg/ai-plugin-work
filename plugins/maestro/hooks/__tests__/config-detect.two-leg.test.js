...
/**
 * WP-09: two-leg require for maestro's factories/envConfig escapes.
 *
 ...
 * scripts/lib/runtime/envconfig-lite first; config-cli.js degrades to the
 * vendored detect-only pass when factories/ is absent. These tests simulate
 * the cache install by copying ONLY the plugin-shaped files into a tmp dir
 * (no factories/ above it) and spawning the real entrypoints.
 */
...
function makeCacheInstall() {
  const base = fs.mkdtempSync(path.join(os.tmpdir(), 'maestro-cache-install-'));
  ...
  fs.cpSync(
    path.join(PLUGIN_ROOT, 'scripts', 'lib', 'runtime'),
    path.join(plugin, 'scripts', 'lib', 'runtime'),
    { recursive: true }
  );
  fs.copyFileSync(
  ...
  fs.writeFileSync(
    ...
    JSON.stringify({
      plugin: 'maestro',
      vars: { MAESTRO_TEST_FAKE_VAR_WP09: { description: 'fixture-only var' } },
    })
  );
...
function spawnEntry(file, { input = '', home, cwd, env = {} }) {
  ...
  delete childEnv.MAESTRO_TEST_FAKE_VAR_WP09;
...
test('config-detect runs standalone in a cache-isolated install (no factories/)', () => {
  ...
  assert.match(r.stdout, /⚙ maestro: 1 unconfigured config var\(s\): MAESTRO_TEST_FAKE_VAR_WP09/);
  assert.match(r.stdout, /Run \/maestro:configure to set them up/);
});
...
test('config-cli degrades to the vendored detect-only pass in a cache install', () => {
  ...
  assert.match(r.stdout, /MAESTRO_TEST_FAKE_VAR_WP09/);
});
