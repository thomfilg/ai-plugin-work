...
| `CONDUCT_WAKE_EVENTS` | (15-kind set) | Comma-separated allowlist of event kinds that wake the conductor model. Default (all 15, explicit): `question-pending`, `nudges-exhausted`, `wedged`, `dead-end`, `dead-end-probe`, `pr-ready`, `pr-broken`, `pr-comments-stuck`, `comment-loop`, `stuck-input`, `auth-broken`, `spinner-hang`, `no-progress`, `kill-during-ci`, `stop-condition-met`. `HEARTBEAT` and `log-only` info lines are excluded so an idle fleet does not burn model turns. Unknown kinds never match (fail-closed to "does not wake"). A **custom list REPLACES the default entirely** — omitting a kind silences it, so start from the full default and add/remove. `all` or `*` restores the pre-680 always-wake firehose. |
| `PENDING_REWAKE_MIN` | 30 | Re-wake throttle floor (minutes). The FIRST emission of an alert key (`session\|kind\|sha-or-phase`) always wakes immediately; repeats of the same key re-wake only after this backoff, which doubles per re-wake up to `PENDING_REWAKE_MAX_MIN`. Throttled repeats still land in `maestro-alerts.jsonl`, the tmux alert pane, and the UserPromptSubmit banner. `0` (or `CONDUCT_WAKE_EVENTS=all`) restores wake-on-every-repeat. |
| `PENDING_REWAKE_MAX_MIN` | 240 | Cap (minutes) for the doubling re-wake backoff of an unhandled pending alert. |
