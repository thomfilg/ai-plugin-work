 ...
 * YAML front-matter block, a test fixture describing a review left by a bot —
 * and there `author: '<some-bot>'` is DATA about who reviewed something, not a
 ...
 * of them. `-` is included for the markdown bullet form; a document that needs
 * to quote a raw trailer in prose puts it in a code span, which prose mode
 * blanks, or names the file in `.ghostwriterignore`.
 */
 ...
 * joins two unrelated sentences: "…(created by WP-02). <Tool> payloads…" is
 * a note about a work package followed by a note about a runtime, and matching
 * it credits a tool that the text never mentions as an author.
 */
function gap(limit) {
  return `(?:(?!\\.[\\s)])[^\\n]){0,${limit}}?`;
}
...
/**
 * The ordered attribution rule set, over one tier of trailer keys.
 *
