...
| `CHECK_TYPECHECK_BASELINE` | `1` | Set to `0` to disable the /check typecheck-error delta entirely (no typecheck run, no baseline read/write) even when `SCRIPT_TYPECHECK_COMMAND` is set. The baseline is captured on the first classified run for the ticket (those errors report as pre-existing, never "clean") and ratchets down whenever a run has zero net-new errors |
