...
> argument (e.g. the ticket id) is the text after the `$work-implement` skill
...
> if [ -f "${CLAUDE_PLUGIN_ROOT:-/nonexistent}/skills/work-implement/SKILL.md" ] &&
>    [ "${CLAUDE_PLUGIN_ROOT}/skills/work-implement/SKILL.md" -ef "$SKILL_MD" ]; then
...
1. Commit changes through the sanctioned script — stage YOUR changes first (`git add <paths>`, never `git add -A`), author a semantic message, then run `node ${CLAUDE_PLUGIN_ROOT}/scripts/workflows/work/scripts/commit-and-push.js -m "<message>"` (a raw `git commit` is blocked; the script commits only what you staged — it fails if nothing is staged — enforces format, no attribution, and a human identity, then pushes). Subtasks commit before returning, unlike normal mode.
...
- This command does NOT create commits - after reviewing, `git add <paths>` (never `git add -A`) then commit via `commit-and-push.js` (a raw `git commit` is blocked; the script commits only what you staged and enforces conventions)
...
- **Invocation**: mention `$work-implement` (work-workflow:work-implement); the
  description is the text after the skill mention (no `$ARGUMENTS`
  substitution on codex — see the note at the top).
- **Step 2 (TodoWrite)**: use `update_plan` — same item structure, codex's
  native plan tool.
- **Step 3 (agent delegation)**: there is no Task tool. Read the persona file
  at `${CLAUDE_PLUGIN_ROOT}/agents/<agent-name>.md` (validated root — see the
  preamble), adopt it, and execute the same prompt INLINE in this session. The
  TDD loop stays hook-enforced either way: codex alias-fires the Write/Edit
  hook lane for `apply_patch`, so phase file-gating
  (work-implement-enforce.js) and stop-gating (enforce-tdd-on-stop.js) still
  apply to every edit.
- **File writes**: "Write tool" instructions mean a normal codex file edit
  (applied as `apply_patch`) — verify the file exists and is non-empty
  afterwards, same as on Claude.
- `[work:codex-degraded]` notices in hook/gate output are informational
  fallback notes, not errors.
