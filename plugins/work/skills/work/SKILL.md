...
| `commit` | YOU (the session agent) do it — do NOT dispatch a subagent. Follow the `prompt`: stage YOUR changes (`git add <paths>` — never `git add -A`, which would sweep other agents' unrelated uncommitted work in a shared worktree), author a concise semantic commit message for them (`type(scope): description`, referencing the ticket, **no AI attribution**), then run the sanctioned commit script from the `prompt` — `node "<…>/commit-and-push.js" -m "<your message>"` — which validates, commits what you staged, and pushes. A raw `git commit` is blocked by `enforce-agent-usage`; the script is the only path. If it rejects the message (format, attribution, an AI git identity, or nothing staged), fix it and re-run. |
