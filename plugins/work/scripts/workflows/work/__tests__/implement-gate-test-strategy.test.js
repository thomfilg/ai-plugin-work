...
// ── scenario: Legacy Test-Command task is unchanged with the flag off ────────

describe('legacy ### Test Command with flag OFF', () => {
  it('returns the verbatim legacy command, no synthesis', () => {
    setFlag('0');
    const tasksDir = path.join(tmp, 'GH-TEST');
    const worktreeDir = path.join(tmp, 'wt');
    writeEnvrc(worktreeDir, 'export TEST_UNIT_COMMAND="should-not-be-used"\n');
    writeTasksMd(
      tasksDir,
      [
        '## Task 1 — Legacy task',
    ...
    const cmd = implementGate.readTaskTestCommand(tasksDir, 1, worktreeDir);
    assert.equal(cmd, 'pnpm test legacy.spec.js');

    const res = implementGate.resolveTaskTestExecution(tasksDir, 1, worktreeDir);
    assert.equal(res.source, 'command');
    assert.equal(res.command, 'pnpm test legacy.spec.js');
  });

  it('two-arg readTaskTestCommand is byte-for-byte unchanged (no synthesis)', () => {
    setFlag('0');
    const tasksDir = path.join(tmp, 'GH-TEST');
    writeTasksMd(
      tasksDir,
      [
        '## Task 1 — Strategy-only task, flag off',
        '',
        '### Type',
        'backend',
        '',
    ...
    // Flag off + only a ### Test Strategy → legacy path yields null.
