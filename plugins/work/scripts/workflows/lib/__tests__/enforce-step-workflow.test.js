...
describe('enforce-step-workflow', () => {
  ...
  describe('source structure', () => {
    ...
    it('ticket pattern uses broad [A-Z]+-\\d+ not project-specific prefix', () => {
      ...
      assert.ok(!hookSource.includes('TASK-'), 'Should not have hardcoded project prefix');
    });
      ...
      const { code } = await runHook(
        {
          ...
          transcript_path:
            '/home/u/.claude/projects/-home-u-dev-w-tabwoah-tabwoah-TEST-FALLBACK-TR-9998/abc.jsonl',
        },
        'PreToolUse',
        { ENFORCE_HOOK_TICKET_ID: '' }
      );
