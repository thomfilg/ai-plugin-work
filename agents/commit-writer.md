...
hooks:
  PreToolUse:
    - matcher: ".*"
      hooks:
        - type: command
          command: "node ${CLAUDE_PLUGIN_ROOT}/scripts/workflows/work-orchestrator/agents/commit-writer/commit-writer-block-write.js"
  PostToolUse:
    - matcher: "Bash"
      hooks:
        - type: command
          command: "node ${CLAUDE_PLUGIN_ROOT}/scripts/workflows/work-orchestrator/agents/commit-writer/commit-writer-precommit-guard.js"
---
...
2. Apply these rules:
   ...
   - `scripts/workflows/work-orchestrator/...` → scope = `work`
   - `agents/...` → scope = `agents`
   - `skills/...` → scope = `skills`
   - `hooks/...` → scope = `hooks`
