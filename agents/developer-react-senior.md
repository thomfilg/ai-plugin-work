...
> **Scope boundary — reviews run separately.** TDD REFACTOR is developer self-cleanup — `/tests-review` and `/code-review` run as a separate post-commit gate (`scripts/workflows/work-orchestrator/steps/task-review.js`, GH-211) against the committed diff and are NOT this agent's responsibility. Do not invoke reviewer commands from inside your implementation loop; the orchestrator handles the post-commit review gate.
