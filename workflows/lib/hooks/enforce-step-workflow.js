...
function handlePreToolUse(hookData) {
  ...
  // Rule 0: Block direct writes to follow-up-pr state files in /tmp/.claude/
  // Runs BEFORE ticket context check — these are global gate files, not per-ticket.
  if (['Write', 'Edit', 'MultiEdit'].includes(toolName)) {
    const filePath = toolInput?.file_path || '';
    if (/follow-up-pr.*\.json$/.test(filePath) && filePath.includes(path.join(os.tmpdir(), '.claude'))) {
      const transcriptPath = hookData?.transcript_path;
      if (!isRunningInAgent(transcriptPath, ['follow-up-pr'], hookData)) {
        didBlock = true;
        process.stderr.write(
          `BLOCKED: Cannot write follow-up-pr state file — not running in the follow-up-pr agent.\n` +
          `File: ${filePath}\n` +
          `This file gates the follow_up verification step and cannot be created manually.\n` +
          `Run the /follow-up-pr skill instead.\n`
        );
        process.exit(2);
      }
    }
  ...
  if (toolName === 'Bash') {
    const cmd = String(toolInput?.command || '');
    const tmpClaudeDir = path.join(os.tmpdir(), '.claude');
    // Block ANY Bash command that could create/modify follow-up-pr state files.
    // Catches: redirects, cp, mv, tee, python, node (custom scripts), perl, ruby, etc.
    // Only exception: read-only commands (cat without redirect, ls, test, stat, rm)
    const isReadOnly = /^(cat|ls|test|stat|file|wc|head|tail|less|more|rm)\s/.test(cmd.trim());
    if (!isReadOnly && /follow-up-pr/.test(cmd) && cmd.includes(tmpClaudeDir) && /\.json/.test(cmd)) {
      const transcriptPath = hookData?.transcript_path;
      if (!isRunningInAgent(transcriptPath, ['follow-up-pr'], hookData)) {
        didBlock = true;
        process.stderr.write(
          `BLOCKED: Cannot interact with follow-up-pr state files via Bash.\n` +
          `This file gates the follow_up verification step and cannot be created manually.\n` +
          `Run the /follow-up-pr skill instead.\n`
        );
        process.exit(2);
