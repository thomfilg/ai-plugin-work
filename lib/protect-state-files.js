...
/** Inline interpreter pattern: extracts language and inline code from -c/-e flags */
const INLINE_INTERPRETER_PATTERN = /\b(python[23]?|ruby|perl|node)\s+(?:--?\w[\w-]*(?:=\S+)?\s+)*-[ce]\s+(['"])([\s\S]*?)\2/g;

/**
 * Python write operations.
 * NOTE: `.write(` is intentionally broad (matches sys.stdout.write, etc.).
 * This is a conservative security trade-off: if inline code reads a protected
 * file AND calls .write() on any object, the block fires. Accepted because
 * the alternative (missing real writes) is worse in our threat model.
 */
const PYTHON_WRITE_OPS = /\bopen\s*\(.*['"]w|\.write\s*\(|\.write_text\s*\(|\.write_bytes\s*\(|\bshutil\.(?:copy|move)\b|\bos\.rename\s*\(/;

/** Ruby write operations (File.open only when followed by a write-mode indicator) */
const RUBY_WRITE_OPS = /\bFile\.write\b|\bFile\.open\s*\(.*['"]w|\bIO\.write\b|\bFileUtils\.(?:cp|mv)\b/;

/** Perl write operations */
const PERL_WRITE_OPS = /\bopen\s*\(.*[>]|\bsyswrite\b|\bFile::Copy\b/;

/** Map interpreter names to their write-detection regexes */
const LANG_WRITE_OPS = {
  python: PYTHON_WRITE_OPS,
  python2: PYTHON_WRITE_OPS,
  python3: PYTHON_WRITE_OPS,
  ruby: RUBY_WRITE_OPS,
  perl: PERL_WRITE_OPS,
  node: NODE_FS_WRITES,
};
...
function createFileProtector(opts) {
  ...
  function check(toolName, toolInput, hookData) {
    ...
    if (toolName === 'Bash') {
      ...
      // ── Vector 3b: Inline interpreter bypass — python -c / ruby -e / etc ──
      const inlineResult = checkInlineInterpreter(cmd, toolInput, hookData);
  ...
  /**
   * Vector 3b: Inline interpreter bypass detection.
   * Detects python3 -c "...", ruby -e "...", perl -e "...", node -e "..."
   * and checks if the inline code contains both a write pattern AND a protected file ref.
   *
   * @param {string} cmd — Bash command string
   * @param {object} toolInput
   * @param {object} [hookData]
   * @returns {CheckResult}
   */
  function checkInlineInterpreter(cmd, toolInput, hookData) {
    INLINE_INTERPRETER_PATTERN.lastIndex = 0;
    let m;
    while ((m = INLINE_INTERPRETER_PATTERN.exec(cmd)) !== null) {
      const lang = m[1];
      const inlineCode = m[3];

      const writeOpsRegex = LANG_WRITE_OPS[lang];
      if (!writeOpsRegex || !writeOpsRegex.test(inlineCode)) continue;

      // Check if any protected file basename appears in the inline code
      const tokens = inlineCode.match(/[^\s"'`|;&(){}[\],]+/g) || [];
      for (const token of tokens) {
        const match = isProtected(token);
        if (match && !isExempt('Bash', toolInput, hookData)) {
          return {
